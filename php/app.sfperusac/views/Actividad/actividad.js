...
$(document).ready(function(){
  ...
  //$('#menumant').attr("style", "display:block;")
  //botones para listar
  ...
  //	url: '../../app/controllers/ActividadController.php',
    url: url,
      ...
      j = 0;
        ...
        estado = e.estado;
        tipo = e.tipo1;
        if(descripcion.length>16 && tipo=='S') {
          descripcion = descripcion.substr(0,16)+'...';
        }
        if(descripcion.length>22 && tipo=='N') {
          descripcion = descripcion.substr(0,20)+'...';
        }
...
{
  tablaLabores.clear().draw(false);
  $("#tablaLabores").dataTable().fnDestroy();     
  construyetabla();
}
...
function tab_btn(event,control)
{
  var t = event.keyCode ? event.keyCode : event.which ? event.which : event.charCode;
	if (t == 9) 
	{
   if(evento=="edit" && control=="ACT_Codigo"){
    $('#ACT_Nombre_Corto').focus();
...
$('#tablaLabores_length').val(select);
